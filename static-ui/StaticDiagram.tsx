...
import { useEdgeHighlighting } from '../webview-ui/hooks/useEdgeHighlighting';
...
function StaticDiagramContent({ data, route, onBack }: StaticDiagramProps): JSX.Element {
  ...
  const interaction = useMemo<DiagramInteractionContextValue>(() => ({
    highlightedColumns: highlighting.highlightedColumns, selectedTableId: selection?.kind === 'table' ? selection.id : null,
    selectedColumnRef: selection?.kind === 'column' ? selection : null, selectedColumns: new Map(), cutColumns: new Map(), insertionTarget: null,
    onColumnHover: highlighting.onColumnHover, onColumnLeave: highlighting.onColumnLeave, onTableSelect: (id) => setSelection({ kind: 'table', id }),
