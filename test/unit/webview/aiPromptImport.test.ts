...
function host(text: string): AiPromptImportHost & { edits: unknown[]; completedCalls: [number, number][]; failures: string[] } {
  ...
  return { findModel: () => model, clipboard: { paste: async () => text }, notifier: { completed: async (updated, rejected) => { completedCalls.push([updated, rejected]); }, failed: async (message) => { failures.push(message); } }, applyAndPersist: async (edit) => { edits.push(edit); }, edits, completedCalls, failures };
}
